...
#define ALIGNED_PUSH(esp, src, type) \
  {                                  \
    esp -= (char *)WORD_SIZE;        \
    **esp = (type)src;               \
  }
...
bool load(const char *file_name, void (**eip)(void), void **esp)
{
  ...
  if (file_read(file, &ehdr, sizeof ehdr) != sizeof ehdr || memcmp(ehdr.e_ident, "\177ELF\1\1\1", 7) || ehdr.e_type != ET_EXEC // Executable file flag
